...
public class RpmEngineService {
    ...
    public RpmMemoria registrarMemoria(Long analisisId,RpmMemoriaRequest req){
        ...
        try{m.setSituacionJson(objectMapper.writeValueAsString(Map.of("servicioId",a.getServicio().getId(),"control",a.getSoaControl()!=null?a.getSoaControl().getControl().getCodigo():"","riesgo",a.getRiesgo()!=null?a.getRiesgo().getCodigo():"","senales",senalRepository.findByAnalisisIdOrderByPesoDesc(a.getId()).stream().map(RpmSenal::getCodigo).toList())));}catch(Exception e){m.setSituacionJson("{}");}

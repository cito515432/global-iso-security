...
function renderRpm() {
  ...
  $("rpmList").innerHTML = data.length ? data.map(({analysis:a, decision:d}) => {
    ...
    return `<div class="card-app mb-3"><div class="d-flex justify-content-between gap-3"><div><h3>${App.esc(a.controlCodigo || "Análisis RPM")} · ${App.esc(a.controlTitulo || "")}</h3><p class="small-app text-muted-app">${App.esc(a.explicacion || a.resumen)}</p></div><div><small class="text-muted-app">RPM</small><br>${App.badge(a.prioridad)} <strong>${a.puntaje}/100</strong></div></div>${App.mlPanel(a,true)}<div class="signal-list">${(a.senales || []).map(s => `<span class="signal" title="${App.esc(s.descripcion)}">${App.esc(s.codigo)} +${s.peso}</span>`).join("")}</div><div class="priority-card"><h4>${App.badge(d.estado)} ${App.esc(App.pretty(d.tipoAccion))}</h4><p>${App.esc(d.accion)}</p>${d.justificacion ? `<p><strong>Validación:</strong> ${App.esc(d.justificacion)}</p>` : ""}<div class="mt-2">${actions}</div></div></div>`;
